...
async def test_escalation_failure_records_pending(factory, tmp_path, monkeypatch):
    """When escalation leaves pages pending, the run completes but records the page
    ranges in escalation_pending (→ warning + retry-eligible). The full retry flow
    (cache reuse → re-escalate → re-split → re-persist) is covered end-to-end in
    ...
    async with factory() as s:
        ...
        assert run.escalation_pending == [{"page_start": 0, "page_end": 0}]
