"""Dell support-manuals documentation profile.

Dell hosts product guides at ``www.dell.com/support/manuals/<lang>/<family>/<guide>``
(e.g. PowerProtect Data Manager). Two things make these hard:

* **Akamai blocks our scraper egress.** A cold Firecrawl scrape (and a plain
  Browserless render) gets an Akamai "Access Denied" page, not the content. We
  defeat it with a **warm-up navigation** (visit ``dell.com`` first so Akamai's
  JS sets its clearance cookies in the session, then load the target) — see
  :meth:`Scraper.warmup_render`. Because Firecrawl can't do that warm-up, both
  TOC discovery *and* content scraping go through Browserless
...
def _to_en_us(href: str, base_url: str) -> str:
    ...
    topic key) is preserved; the localised slug is left as-is since Dell resolves
    the topic by guid regardless."""
...
def parse_dell_toc(html: str, root_url: str) -> list[TocEntry]:
    """Parse Dell's ``#toc-main-parent-ul`` list-group into an ordered TOC.
...
class DellProfile:
    name = "dell"
    # Akamai blocks Firecrawl; both TOC and content go through Browserless with a
    # warm-up navigation (see module docstring).
    render_engine = "browserless"

    def detect(self, root_html: str, root_url: str) -> bool:
        """Match by URL — Dell's root HTML, when fetched cold, is the Akamai block
        page, so an HTML fingerprint can't be relied on."""
        p = urlparse(root_url)
        return p.netloc.endswith("dell.com") and "/support/manuals/" in p.path
    ...
    async def build_toc(self, root_url: str, scraper) -> list[TocEntry]:
        ...
        return parse_dell_toc(html, root_url)

    def browserless_content_spec(self) -> dict:
        """Tell the Browserless content path how to fetch a Dell article body:
        warm up past Akamai, then take the innerHTML of ``#divTopicContent``."""
