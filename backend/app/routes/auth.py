...
_OAUTH_PROVIDERS = {
    ...
    "okta": {
        # authorize/token URLs are built from auth_okta_domain at call time
        "scope": "openid email profile",
    },
}
...
# ---------------------------------------------------------------------------
# User registration (admin-only for now; can be opened up later)
# ---------------------------------------------------------------------------
...
async def register_user(
    ...
    """Register a new local user. In production this would typically be
    admin-only or behind an invite flow; for MVP it's open but rate-limited
...
async def create_api_key(
    ...
    # Build response — raw_key is only in the response, not the ORM model
...
async def list_api_keys(
    ...
    """List the current user's API keys (without revealing the key values)."""
...
async def oauth_authorize(provider: str):
    """Return the authorization URL for the external OAuth2 provider."""
    _auth_disabled()
...
async def oauth_callback(
    ...
    if not user:
        ...
        # Update OAuth linkage if it changed
        if user.oauth_provider != provider or user.oauth_subject != provider_subject:
