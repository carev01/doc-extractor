...
- **Image enrichment never touches `content_hash`**: `content_hash` is a raw-scrape change-detection fingerprint (already decoupled from the served `content_markdown`). `enrich_run_images` edits `content_markdown` + `ArticleImage` fields and emits an `updated` row, but must NOT recompute `content_hash` (that would break the unchanged-page fast path and cause phantom deltas). No-churn is enforced by the `is_meaningful IS NULL OR description IS NULL` candidate predicate; `ArticleImage.is_meaningful` is a nullable tri-state (`NULL` = unevaluated), never defaulted.
