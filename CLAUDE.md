...
- **Oxygen's post-scrape TOC rebuild keys nodes by `(url, k)`, and applies atomically**: `oxygen_webhelp.rebuild_toc` stitches per-page nav fragments, and the obvious identities are all wrong. `data-tocid` embeds a publishing-build id, so a portal served from several builds hands one topic several tocids — Rubrik RSC's rebuilt TOC held every section **three times** (9,268 entries for 4,891 pages) and nobody saw it because the run "succeeded". URL alone merges an Oxygen heading into the child whose page it borrows (Security → Users and Access both link `users_access.html`; 71 such pairs), flattening a level; URL + title fails because 124 URLs are retitled between builds. `k` = consecutive ancestors sharing the URL: the heading is `(url, 0)` and emitted URL-less, the topic `(url, 1)` owns the page. Children are an ordered union across fragments (not "longest list wins", which orphaned 168 nodes to the top level), strays attach under their known parent, and fragments are processed in URL order because the caller reads them without ORDER BY. Known limit: a topic reused under two parents appears once, and children seen only in its other context follow it (RSC: ≤97 children on 26 pages). Applying the result goes through `_apply_rebuilt_toc`: a **savepoint**, never `db.rollback()` (which expires every instance and makes `extract_source`'s later `run.`/`source.` reads a `MissingGreenlet` crash), and the re-link is a **Core** executemany on `Article.__table__` — an ORM `update(Article)` with a parameter list is "bulk UPDATE by primary key" and cannot key on URL (it raised on every run from 2026-06-29; `synchronize_session=None` only changes the error).
