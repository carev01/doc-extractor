...
- **Extraction resilience** — A run whose table of contents collapses (a scraper/upstream failure) fails loudly instead of mass-removing good content, and pages blocked by bot protection (Akamai/Cloudflare) are auto-retried when they're a small fraction of the run, or on demand via a "retry blocked pages" action — mirroring the PDF-escalation and image-enrichment retry flows.
